...
//-------------------------------------------------------------------------//
// Test of finding a patch bounding box in 3D
//-------------------------------------------------------------------------//

using namespace dealii;

void patch_bounding_box_test_3D_linear_mesh(const int order)
{
  const int dim = 3;
  ...
  std::cout << "Min z = " << coord_min(2) << std::endl;
  std::cout << "Max x = " << coord_max(0) << std::endl;
  std::cout << "Max y = " << coord_max(1) << std::endl;
  std::cout << "Max z = " << coord_max(2) << std::endl;
}

void patch_bounding_box_test_3D_general_mesh_nodal(const int order)
{
  const int dim = 3;
...
void patch_bounding_box_test_3D_general_mesh_elemental(const int order)
{
  const int dim = 3;
...
int main()
{
  patch_bounding_box_test_3D_linear_mesh(1);
  patch_bounding_box_test_3D_linear_mesh(2);

  // Still testing on linear meshes, but the bounding box function does not
  // assume they are linear in its implementation.
  patch_bounding_box_test_3D_general_mesh_nodal(1);
  patch_bounding_box_test_3D_general_mesh_nodal(2);

  patch_bounding_box_test_3D_general_mesh_elemental(1);
  patch_bounding_box_test_3D_general_mesh_elemental(2);
}